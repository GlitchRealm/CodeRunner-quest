...
export class GameInitialization {
        ...
        try {
            ...
            if (this.game.settingsSystem) {
                ...
                if (showLoadingScreen !== undefined) {
                     ('🔄 Loading screen setting from SettingsSystem:', showLoadingScreen);
            ...
            if (settings.showLoadingScreen !== undefined) {
                 ('🔄 Loading screen setting from localStorage:', settings.showLoadingScreen);
    ...
    getShouldShowOpeningAnimation() {
        try {
            ...
            if (forceLoginAfterSignout === 'true') {
                 ('🎬 Forcing opening animation after sign out');
                return true;
            }
            ...
            if (this.game.settingsSystem) {
                ...
                if (showOpeningAnimation !== undefined) {
                     ('🎬 Opening animation setting from SettingsSystem:', showOpeningAnimation);
            ...
            if (settings.showOpeningAnimation !== undefined) {
                 ('🎬 Opening animation setting from localStorage:', settings.showOpeningAnimation);
    ...
    async initAsync() {
        try {
            ...
            if (this.game.settingsSystem) {
                 ('⚙️ Loading settings before navigation decisions...');
            ...
            const shouldShowOpeningAnimation = this.getShouldShowOpeningAnimation();
             ...
             ('🎬 DEBUG: shouldShowLoadingScreen =', shouldShowLoadingScreen);
             ('🎬 DEBUG: shouldShowOpeningAnimation =', shouldShowOpeningAnimation);
             ('🎬 DEBUG: settingsSystem exists =', !!this.game.settingsSystem);
            if (this.game.settingsSystem) {
                ...
                const openingAnimationSetting = this.game.settingsSystem.getSettingValue('showOpeningAnimation');
                 ('🎬 DEBUG: showLoadingScreen setting value =', loadingScreenSetting);
                 ('🎬 DEBUG: showOpeningAnimation setting value =', openingAnimationSetting);
            }
            
            // Determine the target state after loading/initialization
            let targetState = GAME_STATES.HOME;
            if (shouldShowOpeningAnimation) {
                 ('🎬 Opening animation enabled - will transition to opening animation');
                targetState = GAME_STATES.OPENING_ANIMATION;
            }
            
            if (shouldShowLoadingScreen) {
                 ('🔄 Loading screen enabled - setting LOADING state');
                this.game.gameState = GAME_STATES.LOADING;
                this.game.pendingGameState = targetState;
                 ('🔄 DEBUG: Set pendingGameState to:', targetState);
            } else {
                 ('⏭️ Loading screen disabled - going directly to target state:', targetState);
            ...
            // ❌ Game initialization failed (log removed)
    ...
    async init() {
        ...
        // Load saved game data (excluding settings which are already loaded)
         ('🎮 Game.init() about to call loadGameData()');
        await this.loadGameData();
         ('🎮 Game.init() finished loadGameData()');
    ...
    createSystems() {
        // 🛡️ Initialize error handling and performance monitoring first
        this.game.errorHandler = new GameErrorHandler(this.game);
        this.game.performanceMonitor = new PerformanceMonitor(this.game);
        ...
        try {
            this.game.powerUpSystem = new PowerUpSystem(this.game);
             ('✅ PowerUpSystem initialized successfully');
        } catch (error) {
            // ❌ PowerUpSystem initialization failed (log removed)
        ...
        this.game.canvas.addEventListener('focus', () => {
            ...
            if (this.game.gameState === GAME_STATES.PLAYING || this.game.gameState === GAME_STATES.PAUSED) {
                document.body.classList.add('game-focused');
                 ('🎮 Canvas focused during gameplay - preventing page scroll');
            } else {
                 ('🎮 Canvas focused in menu state - allowing page scroll');
            }
        });
        
        this.game.canvas.addEventListener('blur', () => {
            document.body.classList.remove('game-focused');
             ('🎮 Canvas blurred - allowing page scroll');
        });
    }
    ...
    ensureCanvasFocus() {
        try {
            ...
            if (this.game.gameState === GAME_STATES.PLAYING || this.game.gameState === GAME_STATES.PAUSED) {
                document.body.classList.add('game-focused');
                 ('🎮 Preventing scroll for gameplay');
            } else {
                // Remove the class to allow scrolling in menus
                document.body.classList.remove('game-focused');
                 ('🎮 Allowing scroll for menu state:', this.game.gameState);
            }
            
            // Focus the canvas to ensure keyboard events are captured
            this.game.canvas.focus({ preventScroll: true });
            
             ('🎮 Canvas focused for keyboard input');
        } catch (error) {
    ...
    initializeGraphicsSettings() {
        if (window.generalSettings) {
            ...
            this.game.showFpsCounter = window.generalSettings.isShowFpsCounterEnabled();
             ('🎮 Graphics settings initialized - FPS counter:', this.game.showFpsCounter);
            ...
            this.game.showFpsCounter = false;
             ('🎮 Using default graphics settings - FPS counter:', this.game.showFpsCounter);
        }
    }
        ...
        if (this.game.gameState === GAME_STATES.SHOP) {
            ...
            if (this.game.characterCustomizationSystem && this.game.characterCustomizationSystem.handleWheel) {
                this.game.characterCustomizationSystem.handleWheel(e.deltaY);
                 ('🎭 Character customization scroll:', e.deltaY);
            }
    ...
    handleWindowFocus() {
        ...
        if (this.game.pauseOnFocusLoss && this.game.wasAutoPaused) {
            ...
            this.game.isPaused = false;
             ('🎮 Window focused - resuming auto-paused game');
        }
    }
    ...
    handleWindowBlur() {
        ...
        if (this.game.pauseOnFocusLoss && this.game.gameState === GAME_STATES.PLAYING && !this.game.isPaused) {
            ...
            this.game.isPaused = true;
             ('🎮 Window blurred - auto-pausing game');
        }
    }
    ...
    ensureProperScrollBehavior() {
        if (this.game.gameState === GAME_STATES.PLAYING || this.game.gameState === GAME_STATES.PAUSED) {
            document.body.classList.add('game-focused');
             ('🔒 Initial state: Preventing page scroll for gameplay');
        } else {
            document.body.classList.remove('game-focused');
             ('📜 Initial state: Allowing page scroll for menu state:', this.game.gameState);
        }
    }
                ...
                toggleAudio: () => {
                    if (this.game.audioSystem) {
                        this.game.audioSystem.toggleMute();
                         ('Audio muted:', this.game.audioSystem.getIsMuted());
                    }
                },
                setVolume: (volume) => {
                    if (this.game.audioSystem) {
                        this.game.audioSystem.setMasterVolume(volume);
                         ('Master volume set to:', volume);
                    }
                }
            };
             ('🐛 Debug commands added to window.gameDebug');
        }
        ...
        window.debugCharacterSelection = {
            getCurrentSprite: () => {
                if (window.profileManager) {
                    const current = window.profileManager.getSelectedSprite();
                     ('Current sprite in ProfileManager:', current);
                    return current;
                }
            },
            setSprite: (spriteId) => {
                if (window.profileManager) {
                     ('Setting sprite to:', spriteId);
            ...
            getCurrentPlayerSprite: () => {
                if (window.game && window.game.player && window.game.player.sprite) {
                     ('Current player sprite src:', window.game.player.sprite.src);
            ...
            checkKingRunner: () => {
                if (this.game.characterCustomizationSystem) {
                    return this.game.characterCustomizationSystem.debugKingRunnerStatus();
                     ...
                     ('❌ Character customization system not available');
                }
            }
        };
    }
        ...
        try {
            ...
            if (savedScores) {
                this.game.bestScores = JSON.parse(savedScores);
                 ('📊 Best scores loaded:', this.game.bestScores);
            }
        } catch (error) {
    ...
    async loadGameData() {
        ...
        try {
            ...
            // ❌ Could not load game data (log removed)
        }
    }
    ...
    async determineInitialNavigation() {
        ...
        try {
            // This would check if user is logged in and navigate accordingly
             ('🔑 Determining initial navigation...');
            ...
            if (this.game.gameState === GAME_STATES.LOADING) {
                ...
                if (!this.game.pendingGameState) {
                     ('🔑 No pending state set, defaulting to HOME');
                    this.game.pendingGameState = GAME_STATES.HOME;
                } else {
                     ('🔑 Pending state already set to:', this.game.pendingGameState);
                }
            } else {
                this.game.gameState = GAME_STATES.HOME;
            }
        } catch (error) {
            // ❌ Could not determine initial navigation (log removed)
            this.game.gameState = GAME_STATES.HOME;
        }
    }
}
