...
export class CloudSaveSystem {
    ...
    async mergeLocalAndCloudData(localData, cloudData) {
        try {
            const mergedData = {
                ...
                // Merge upgrade system data (take the higher levels for each upgrade type)
                upgradeSystemData: this.mergeUpgradeSystemData(localData.upgradeSystemData || {}, cloudData.upgradeSystemData || {}),
    ...
    mergeUpgradeSystemData(local, cloud) {
        // Take the higher level for each upgrade type (player should keep their best progress)
        return {
            jumpHeightLevel: Math.max(local.jumpHeightLevel || 0, cloud.jumpHeightLevel || 0),
            scoreMultiplierLevel: Math.max(local.scoreMultiplierLevel || 0, cloud.scoreMultiplierLevel || 0),
            powerUpDurationLevel: Math.max(local.powerUpDurationLevel || 0, cloud.powerUpDurationLevel || 0),
            jumpHeightBonus: Math.max(local.jumpHeightBonus || 0, cloud.jumpHeightBonus || 0),
            scoreMultiplierBonus: Math.max(local.scoreMultiplierBonus || 0, cloud.scoreMultiplierBonus || 0),
            powerUpDurationBonus: Math.max(local.powerUpDurationBonus || 0, cloud.powerUpDurationBonus || 0)
        };
    }
    ...
    async saveUserProfile(profileData) {
        if (!this.isUserLoggedIn() || !this.isInitialized) {
            // User not logged in or Firebase not initialized - saving profile locally
        ...
        try {
            ...
            try {
                ...
                // Server timestamp not available, using local timestamp
            }
            
            await docRef.set(cloudProfileData, { merge: true });
            // User profile saved to cloud successfully
            ...
            // ❌ Error saving user profile to cloud (log removed)
    ...
    async loadUserProfile() {
        if (!this.isUserLoggedIn() || !this.isInitialized) {
            // User not logged in or Firebase not initialized - loading profile locally
        ...
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                // User profile loaded from cloud successfully
                ...
                // No cloud profile found, checking local storage
                return this.loadProfileFromLocalStorage();
            }
            
        } catch (error) {
            // ❌ Error loading user profile from cloud (log removed)
    ...
    saveProfileToLocalStorage(profileData) {
        try {
            ...
            // ❌ Error saving profile to localStorage (log removed)
            return false;
        }
    }
    ...
    loadProfileFromLocalStorage() {
        try {
            ...
            // ❌ Error loading profile from localStorage (log removed)
            return null;
        }
    }
    ...
    async syncUserStats(userStats) {
        ...
        try {
            ...
            await docRef.set(cloudStatsData, { merge: true });
             ('📊 User stats synced to cloud successfully');
            ...
            // ❌ Error syncing user stats to cloud (log removed)
    ...
    async loadUserStats() {
        ...
        try {
            ...
            if (doc.exists) {
                const data = doc.data();
                 ('📊 User stats loaded from cloud successfully');
                ...
                return data;
                 ...
                 ('📊 No cloud stats found, checking local storage');
                return this.loadStatsFromLocalStorage();
            }
            
        } catch (error) {
            // ❌ Error loading user stats from cloud (log removed)
    ...
    saveStatsToLocalStorage(statsData) {
        try {
            localStorage.setItem('coderunner_user_stats', JSON.stringify(statsData));
             ('💾 User stats saved to localStorage');
            return true;
        } catch (error) {
            // ❌ Error saving stats to localStorage (log removed)
            return false;
        }
    }
    ...
    loadStatsFromLocalStorage() {
        try {
            ...
            // ❌ Error loading stats from localStorage (log removed)
            return null;
        }
    }
    ...
    collectGameData() {
        ...
        if (this.game.upgradeSystem) {
            const upgradeSystemData = this.game.upgradeSystem.getSaveData();
            gameData.dataPackets = upgradeSystemData.dataPackets || 0;
            
            // Collect upgrade levels and bonuses - these are critical persistent data!
            gameData.upgradeSystemData = {
                jumpHeightLevel: upgradeSystemData.jumpHeightLevel || 0,
                scoreMultiplierLevel: upgradeSystemData.scoreMultiplierLevel || 0,
                powerUpDurationLevel: upgradeSystemData.powerUpDurationLevel || 0,
                jumpHeightBonus: upgradeSystemData.jumpHeightBonus || 0,
                scoreMultiplierBonus: upgradeSystemData.scoreMultiplierBonus || 0,
                powerUpDurationBonus: upgradeSystemData.powerUpDurationBonus || 0
            };
             (`💾 Collecting upgrade system data for cloud save:`, gameData.upgradeSystemData);
        }
    ...
    applyGameData(gameData) {
        ...
        try {
            // 🔒 Validate and sanitize game data before applying
            const validation = DataValidator.validateGameData(gameData);
            
            if (!validation.isValid) {
                console.warn('⚠️ Game data validation failed:', validation.errors);
                 ('🔧 Using sanitized data instead');
            }
            
            const dataToApply = validation.sanitized;
            
            // Check for data corruption
            const corruptionIssues = DataValidator.detectDataCorruption(gameData);
            if (corruptionIssues.length > 0) {
                console.warn('🚨 Potential data corruption detected:', corruptionIssues);
            }
            
            // Apply best scores
            if (dataToApply.bestScores) {
                this.game.bestScores = { ...this.game.bestScores, ...dataToApply.bestScores };
                 ('✅ Best scores applied from cloud save');
            }
            
            // Apply upgrade data
            if (this.game.upgradeSystem) {
                const upgradeData = {
                    dataPackets: dataToApply.dataPackets || 0
                };
                
                // Apply upgrade system data (levels and bonuses) - critical fix!
                if (dataToApply.upgradeSystemData) {
                    Object.assign(upgradeData, dataToApply.upgradeSystemData);
                     (`💾 Applying validated upgrade system data:`, dataToApply.upgradeSystemData);
                }
                
                this.game.upgradeSystem.loadSavedData(upgradeData);
                 (`✅ Upgrade system data applied from cloud save: ${upgradeData.dataPackets} packets, levels: J${upgradeData.jumpHeightLevel || 0}/S${upgradeData.scoreMultiplierLevel || 0}/P${upgradeData.powerUpDurationLevel || 0}`);
            }
            
            // Apply shop upgrade data
            if (dataToApply.ownedUpgrades && this.game.shopSystem) {
                this.game.shopSystem.loadOwnedUpgrades(dataToApply.ownedUpgrades);
                 (`✅ Shop upgrades applied from cloud save: ${dataToApply.ownedUpgrades.length} upgrades`);
            }
            ...
            if (gameData.profileData && typeof window !== 'undefined' && window.profileManager) {
                window.profileManager.loadFromCloud(gameData.profileData);
                 ('✅ Profile data applied from cloud save');
            }
            ...
            if (gameData.audioSettings && this.game.audioSystem) {
                ...
                this.game.audioSystem.saveSettings();
                 ('✅ Audio settings applied from cloud save');
            }
            ...
            if (gameData.leaderboardData && this.game.leaderboardSystem) {
                ...
                if (gameData.leaderboardData.uploadedDifficulties) {
                 ...
                 ('✅ Leaderboard data applied from cloud save');
            }
            ...
            if (gameData.userStats && this.game.userProfileSystem) {
                this.game.userProfileSystem.userStats = { 
                 ...
                 ('✅ User stats applied from cloud save');
            }
            
            return true;
        } catch (error) {
            // ❌ Error applying game data (log removed)
            return false;
        }
    }
    ...
    async saveAllGameData() {
        try {
            ...
            // ❌ Error during comprehensive cloud save (log removed)
            return false;
        }
    }
    ...
    async loadAllGameData() {
        try {
             ('📥 Starting comprehensive cloud load...');
            ...
            if (gameData) {
                ...
                if (success) {
                     ('✅ Comprehensive cloud load completed successfully');
                    return true;
                } else {
                 ...
                 ('ℹ️ No cloud save data found');
                return false;
            }
        } catch (error) {
            // ❌ Error during comprehensive cloud load (log removed)
            return false;
        }
    }
    ...
    startAutoSave() {
        ...
        if (!this.isUserLoggedIn()) {
             ('ℹ️ Auto-save not started - user not logged in');
            return;
        }
        ...
        this.autoSaveInterval = setInterval(async () => {
         ...
         ('✅ Auto-save started (every 2 minutes)');
    }
    ...
    async syncWithCloud() {
        ...
        try {
            ...
            // ❌ Error during cloud sync (log removed)
            return false;
        }
    }
}
